...
function ChucklePostAI(AI_option) {
  ...
  function MAIN(AI_option) {
    ...
    const currentURL = "https://blog.suzukif.xyz" + currentPath;
    // 排除页面
    ...
    async function recommendList() {
      ...
      if(sessionStorage.getItem('recommendList')){
        ...
        try {
          response = await fetch(`https://gemini-stellar.suzukif.xyz/recommends?url=${encodeURIComponent(window.location.href)}&author=${AI_option.rec_method ? AI_option.rec_method : 'all'}`, options);
    ...
    async function matrixShuttle(){
      ...
      if(sessionStorage.getItem('matrixShuttle')){
        ...
        try {
          response = await fetch('https://gemini.suzukif.xyz/websites_used', options);
          completeGenerate = true;
          if (response.status === 429) {
            startAI('请求过于频繁，请稍后再请求AI。');
          }
